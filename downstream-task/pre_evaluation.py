...
# prepare sequence task
    ...
    seq_embedding = get_seq_emb_from_traj_withALLModel(seq_model, test_data, batch_size=550, source_city=sc)
    # seq_embedding = get_seq_emb_from_traj_withRouteOnly(seq_model, test_data, batch_size=1024, source_city=sc)
...
def process_row(row, k):
    ...
    # 删除cpath_list和road_timestamp的最后k项
    pre_label = cpath_list[-k:]
    cpath_list = cpath_list[:-k]
    road_timestamp = road_timestamp[:-k]
    road_interval = road_interval[:-k]

    # 获取road_timestamp的最后一个值
    t = road_timestamp[-1] if len(road_timestamp) > 0 else None
    ...
    # 删除grid_timestamp中大于t的项
    if t is not None:
        grid_timestamp = [gt for gt in grid_timestamp if gt <= t]
        cgrid_list = cgrid_list[:len(grid_timestamp)-1]
        grid_fea = grid_fea[:len(grid_timestamp)-1]
        grid_interval = grid_interval[:len(grid_timestamp)-1]
...
if __name__ == '__main__':
    ...
    # city, source_city = 'chengdu', False
