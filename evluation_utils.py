...
def get_road_embedding3(road_list, gps_road_joint_rep, route_road_joint_rep, gps_road_joint_rep_share, route_road_joint_rep_share, route_assign_mat, seq_model, source_city):
    # 处理在训练时被观测到的路段
    road_joint_rep = torch.cat([gps_road_joint_rep.unsqueeze(2), route_road_joint_rep.unsqueeze(2)], dim=2)
    road_joint_rep_share = torch.cat([gps_road_joint_rep_share.unsqueeze(2), route_road_joint_rep_share.unsqueeze(2)], dim=2)
    if source_city:
        road_embedding = torch.zeros((seq_model.node_embedding.weight.shape[0], road_joint_rep.shape[-1])).cuda()
    else:
        road_embedding = torch.zeros((seq_model.node_embedding_extra.weight.shape[0], road_joint_rep.shape[-1])).cuda()
    route_assign_mat = route_assign_mat[:road_joint_rep.shape[0]]
    for road_id in road_list:
        indexes = torch.nonzero(route_assign_mat==road_id)
        rep_list = [road_joint_rep[index[0]][index[1]] for index in indexes] + [road_joint_rep_share[index[0]][index[1]] for index in indexes]
        road_rep = torch.cat(rep_list, dim=0)
        road_rep = torch.mean(road_rep, dim=0)
    ...
    if source_city:
        ...
        road_rep, _ = seq_model.encode_route_extra(None, unseen_route_assign_mat, unseen_route_assign_mat)
...
# 从观测到的轨迹生成路段的表示
# 输入的是完整的数据，包括路由与GPS
def get_road_emb_from_traj(seq_model, test_data, without_gps=False, batch_size=1024, update_road='mean', city='chengdu', source_city=True):
    ...
    with torch.no_grad():
        ...
        gps_road_joint_rep_list_share, route_road_joint_rep_list_share = [], []
        for i in range(route_data.shape[0] // batch_size + 1):  # 最后不足batch_size的case不要了
            ...
            _, _, _, _, _, _, _, _, _, _, _, _, gps_road_joint_rep, route_road_joint_rep, _, _, \
            _, _, _, _, gps_road_joint_rep_share, route_road_joint_rep_share, _, _ \
                = seq_model(batch_route_data, batch_masked_route_assign_mat, batch_gps_data,
                            ...
                            batch_masked_gps_assign_mat_grid, batch_grid_assign_mat, batch_gps_length_grid, source_city
                            )
            ...
            gps_road_joint_rep_share = torch.cat([gps_road_joint_rep_share, padding], dim=1).cpu()
            route_road_joint_rep_share = torch.cat([route_road_joint_rep_share, padding], dim=1).cpu()

            gps_road_joint_rep_list.append(gps_road_joint_rep)
            route_road_joint_rep_list.append(route_road_joint_rep)
            gps_road_joint_rep_list_share.append(gps_road_joint_rep_share)
            route_road_joint_rep_list_share.append(route_road_joint_rep_share)

            del gps_road_joint_rep, route_road_joint_rep, gps_road_joint_rep_share, route_road_joint_rep_share
    ...
    gps_road_joint_rep_share = torch.cat(gps_road_joint_rep_list_share, dim=0).cpu()  # 注意gps_road_joint_rep_list中不能为空
    route_road_joint_rep_share = torch.cat(route_road_joint_rep_list_share, dim=0).cpu()
    ...
    if update_road == 'mean':
        ...
        road_embedding = get_road_embedding3(road_list, gps_road_joint_rep, route_road_joint_rep, gps_road_joint_rep_share, route_road_joint_rep_share, route_assign_mat,
                                             seq_model, source_city)
...
# 从观测到的轨迹生成轨迹的表示
# 输入的是完整的数据, 包括gps
def get_seq_emb_from_traj_withALLModel(seq_model, test_data, without_gps=False, batch_size=1024, source_city=True):
    ...
    with torch.no_grad():
        ...
        gps_traj_joint_rep_list_share, route_traj_joint_rep_list_share, gps_traj_grid_joint_rep_list_share, grid_traj_joint_rep_list_share = [], [], [], []
        for i in range(route_data.shape[0] // batch_size + 1):  # 最后不足batch_size的case不要了
            if i%20 == 0:
                print(i, route_data.shape[0] // batch_size + 1)
            start_idx = i * batch_size
            ...
            if end_idx > route_data.shape[0]:
                end_idx = None
            if start_idx >= route_data.shape[0]:
                break
            batch_route_data = route_data[start_idx:end_idx].cuda()
            batch_masked_route_assign_mat = masked_route_assign_mat[start_idx:end_idx].cuda()
            batch_gps_data = gps_data[start_idx:end_idx].cuda()
            ...
            gps_traj_joint_rep_share, route_traj_joint_rep_share, gps_traj_grid_joint_rep_share, grid_traj_joint_rep_share,  _, _, _, _ \
                = seq_model(batch_route_data, batch_masked_route_assign_mat, batch_gps_data,
                            batch_masked_gps_assign_mat, batch_route_assign_mat, batch_gps_length,
                            batch_grid_data, batch_masked_grid_assign_mat, batch_gps_data_grid,
                            batch_masked_gps_assign_mat_grid, batch_grid_assign_mat, batch_gps_length_grid, source_city
            ...
            del batch_route_data, batch_masked_route_assign_mat, batch_gps_data, batch_masked_gps_assign_mat, batch_route_assign_mat, batch_gps_length, \
                batch_grid_data, batch_masked_grid_assign_mat, batch_gps_data_grid, batch_masked_gps_assign_mat_grid, batch_grid_assign_mat, batch_gps_length_grid
            torch.cuda.empty_cache() # 清空显存
            gps_traj_joint_rep_list.append(gps_traj_joint_rep.cpu())
            route_traj_joint_rep_list.append(route_traj_joint_rep.cpu())
            gps_traj_grid_joint_rep_list.append(gps_traj_grid_joint_rep.cpu())
            grid_traj_joint_rep_list.append(grid_traj_joint_rep.cpu())

            gps_traj_joint_rep_list_share.append(gps_traj_joint_rep_share.cpu())
            route_traj_joint_rep_list_share.append(route_traj_joint_rep_share.cpu())
            gps_traj_grid_joint_rep_list_share.append(gps_traj_grid_joint_rep_share.cpu())
            grid_traj_joint_rep_list_share.append(grid_traj_joint_rep_share.cpu())
    ...
    gps_traj_joint_rep = torch.cat(gps_traj_joint_rep_list, dim=0).cuda()  # 注意gps_road_joint_rep_list中不能为空
    route_traj_joint_rep = torch.cat(route_traj_joint_rep_list, dim=0).cuda()
    gps_traj_grid_joint_rep = torch.cat(gps_traj_grid_joint_rep_list, dim=0).cuda()
    grid_traj_joint_rep = torch.cat(grid_traj_joint_rep_list, dim=0).cuda()

    gps_traj_joint_rep_share = torch.cat(gps_traj_joint_rep_list_share, dim=0).cuda()  # 注意gps_road_joint_rep_list中不能为空
    route_traj_joint_rep_share = torch.cat(route_traj_joint_rep_list_share, dim=0).cuda()
    gps_traj_grid_joint_rep_share = torch.cat(gps_traj_grid_joint_rep_list_share, dim=0).cuda()
    grid_traj_joint_rep_share = torch.cat(grid_traj_joint_rep_list_share, dim=0).cuda()
    if without_gps:
        traj_joint_rep = route_traj_joint_rep
    else:
        traj_joint_rep = torch.cat([gps_traj_joint_rep.unsqueeze(1), route_traj_joint_rep.unsqueeze(1), gps_traj_grid_joint_rep.unsqueeze(1), grid_traj_joint_rep.unsqueeze(1), \
                                    gps_traj_joint_rep_share.unsqueeze(1), route_traj_joint_rep_share.unsqueeze(1), gps_traj_grid_joint_rep_share.unsqueeze(1), grid_traj_joint_rep_share.unsqueeze(1)], dim=2)
        traj_joint_rep = torch.mean(traj_joint_rep, dim=1)

    return traj_joint_rep

def get_seq_emb_from_traj_withRouteOnly(seq_model, test_data, batch_size=1024, source_city=True):
    ...
    with torch.no_grad():
        ...
        for i in range(route_data.shape[0] // batch_size + 1):  # 最后不足batch_size的case不要了
            ...
            if source_city:
                ...
                route_road_rep, route_traj_rep = seq_model.encode_route_extra(batch_route_data, batch_route_assign_mat, batch_masked_route_assign_mat)
...
def get_seq_emb_from_traj_withGridOnly(seq_model, test_data, batch_size=1024, source_city=True):
    _, _, _, _, _, grid_data, masked_grid_assign_mat, _, _, grid_assign_mat, _, _, _ = test_data

    # 都放到显存里面放不下，需要分batch处理
    with torch.no_grad():
        route_traj_rep_list = []
        for i in range(grid_data.shape[0] // batch_size + 1):  # 最后不足batch_size的case不要了
            if i%100 == 0:
                print(i, grid_data.shape[0] // batch_size + 1)
            start_idx = i * batch_size
            end_idx = (i + 1) * batch_size
            if end_idx > grid_data.shape[0]:
                end_idx = None
            batch_route_data = grid_data[start_idx:end_idx].cuda()
            batch_masked_route_assign_mat = masked_grid_assign_mat[start_idx:end_idx].cuda()
            batch_route_assign_mat = grid_assign_mat[start_idx:end_idx].cuda()
            if source_city:
                route_road_rep, route_traj_rep, _ = seq_model.encode_grid(batch_route_data, batch_route_assign_mat, batch_masked_route_assign_mat)
            else:
                route_road_rep, route_traj_rep, _ = seq_model.encode_grid_extra(batch_route_data, batch_route_assign_mat, batch_masked_route_assign_mat)
            del batch_route_data, batch_masked_route_assign_mat, batch_route_assign_mat
            route_traj_rep_list.append(route_traj_rep)
            del route_traj_rep
            # torch.cuda.empty_cache() # 清空显存
